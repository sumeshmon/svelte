<script lang="ts">
    ...
    import { documentHasFile, getDocumentUrl } from '$utils/documents/helper';
...
<Loader {loader}>
    {#if documentList && totalItems > 0}
        {#each documentList as document}
            <div class="ipo__prospectus">
                <div class="ipo__prospectus-title">
                    ...
                    {:else if document.link}
                        {#each document.link as link}
                            <a
                                ...
                                on:click={(event) => handleClick(event, document)}
                                data-disclaimer={document.disclaimer}
                                data-uuid={document.id}>{@html getDocumentTitle(document)}</a
                            >
                        {/each}
                    {:else}
